...
public abstract class Minimax<M extends Move> {
    ...
    public M getBestMove(final int depth) {
        ...
        case NEGAMAX:
            ...
            break;
        case KILLERNEGASCOUT:
            killerNegascout(wrapper, depth, -maxEvaluateValue(), maxEvaluateValue());
            break;
        case NEGASCOUT:
        default:
    ...
    final Comparator<Move> KillerComparator = new Comparator<Move>() {
        ...
        public int compare(Move o1, Move o2) {
            List<Move> killerMoves = Minimax.this.killerMoves.get(Minimax.this.currentDepth);
    ...
    private double killerNegascout(final MoveWrapper<M> wrapper, final int depth, double alpha, double beta) {
        this.nodesCount++;
        ...
        if (moves.isEmpty()) {
            next();
            double score = killerNegascoutScore(true, depth, alpha, beta, b);
            previous();
            return score;
        } else {
            double score;
            ...
            for (M move : moves) {
                makeMove(move);
                score = killerNegascoutScore(first, depth, alpha, beta, b);
                unmakeMove(move);
                if (score > alpha) {
                    alpha = score;
                    bestMove = move;
    ...
    protected double killerNegascoutScore(final boolean first, final int depth, final double alpha, final double beta, final double b) {
        double score = -killerNegascout(null, depth - 1, -b, -alpha);
        if (!first && alpha < score && score < beta) {
            // fails high... full re-search
            score = -killerNegascout(null, depth - 1, -beta, -alpha);
        }
        return score;
    }
